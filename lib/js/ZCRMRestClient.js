...
ZCRMRestClient.generate_access_token_from_refresh_token = function(user_identifier, refresh_token) {
  return ZCRMRestClient.refresh_access_token(user_identifier, refresh_token);
};

ZCRMRestClient.refresh_access_token = function(user_identifier, refresh_token) {
  return new Promise(function(resolve, reject) {
    if (!refresh_token) {
      reject(new Error("refresh token not provided!"));
    }
    if (!user_identifier) {
      reject(new Error("user identifier not provided!"));
    }
